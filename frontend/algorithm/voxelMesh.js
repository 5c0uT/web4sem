/**
 * Описание шести граней кубического вокселя.
 * Каждая грань хранит смещение к соседу и четыре локальные вершины.
 */
...
/**
 * Переводит hex-цвет в нормализованный RGB-массив.
 * Такой формат напрямую подходит для BufferGeometry.
 */
export function hexToRgbNormalized(hex) {
  const normalized = hex.replace("#", "");
  const value = normalized.length === 3 ? normalized.split("").map((symbol) =>
     `${symbol}${symbol}`).join("") : normalized;
...
/**
 * Проверяет, существует ли соседний воксель.
 * Это позволяет не строить внутренние грани.
 */
...
/**
 * Строит геометрию только по внешним граням модели.
 * На выходе возвращаются позиции, цвета, индексы и счетчики для отладки.
 */
...
/**
 * Преобразует пересечение луча с мешем обратно в координаты вокселя.
 * Луч смещается чуть внутрь грани, чтобы выбрать именно попавший блок.
 */
